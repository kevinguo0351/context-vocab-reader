...
### 全量回归（新增）
用 CDP 直连无头 Edge 驱动**生产构建产物**跑了一轮全功能验证，DeepSeek / 欧陆用 CDP `Fetch` 域打桩（含 CORS 预检），**76 条断言全通过、控制台零报错**。详见 [docs/IMPLEMENTATION.md 第 7 节](docs/IMPLEMENTATION.md)。要点：
- **文本层对齐有了量化证据**：把 120 个 span 的矩形上下左右各挪 ±8px 测底下 canvas 的墨迹密度，峰值恰好落在 0 偏移（0.3992，±8px 处降到 0.21/0.23），100% 的 span 底下有墨迹 —— 坑1 未回归。
- 真实鼠标拖选、真实触摸点词、批量标记落点、复习窗口、IndexedDB（vocab / batches / library）、设置持久化、Alt+B、右键菜单、EPUB 翻页与阅读设置持久化，逐项断言通过。

### Docs
- **修正一条过期的已知限制**：文档原称「EPUB 在无头/自动化环境渲染不出来（iframe 高度=0）」，实测该限制已不成立（0.1.1 的显式像素 `renderTo` 修复解决了它，iframe 高度 765px）。EPUB 的点词、iframe 内批量标记、swipe 翻页、CFI 进度恢复现已全部自动化验证。
- 补记三条此前没写进文档的真实限制：**PDF 不记阅读位置**（`library.progress` 只有 EPUB 分支在写）、**扫描件无法查词**（无嵌入文字层则文本层为空）、**中文字符不可点**（`WORD_RE` 只含拉丁字母，有意取舍）。
