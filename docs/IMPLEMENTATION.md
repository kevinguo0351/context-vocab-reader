...
## 7. 已知限制
- **PDF 不记阅读位置**：`library.progress` 只有 EPUB 分支在写（`rendition.on('relocated')`）。PDF 读到第 300 页，下次打开仍从第 1 页开始 —— schema 有这个字段、EPUB 也在用，PDF 侧单纯没实现。
- **扫描件无法查词**：没有嵌入文字层的 PDF（扫描书、图片版）文本层为空，划词/点词/批量全部不可用。渲染正常，只是没有文字可选。判断方法：`getTextContent()` 的 items 为 0。
- **复杂版式 PDF**（双栏论文/公式/表格）文本层会塌，逐词/批量仍可用但复习左栏是整页 run-on 文本（MVP 取舍）。
- **不规则字形级叠印 PDF** 正文去重不彻底（见坑2）。
- **中文不可点**：`mark.js` 的 `WORD_RE = /[A-Za-z'’\-]/`，点中文字符返回 null、不弹面板。英语查词工具的有意取舍；中文书里夹的英文词正常可点。
- 不支持 DRM EPUB、扫描件 OCR。

### 自动化验证覆盖（2026-08-14）
用 CDP 直连无头 Edge 驱动**生产构建产物**做了一轮全量回归，DeepSeek / 欧陆用 CDP `Fetch` 域打桩（含 CORS 预检），共 76 条断言全通过。四套：

| 套件 | 覆盖 |
|---|---|
| PDF × touch / desktop（各 22 条） | 文本层与 canvas 的**偏移扫描对齐验证**（墨迹密度在 dy=0 取峰值）、真实鼠标拖选、真实触摸点词、批量标记落点、复习窗口、IndexedDB vocab/batches/library、设置持久化 |
| EPUB（15 条） | 渲染、iframe 内点词、swipe 翻页、CFI 进度存取、iframe 内批量标记**词命中正确性**、重开恢复位置 |
| 交互（17 条） | Alt+B、右键菜单（含定位与外部点击关闭）、EPUB 上/下一页、字号/行距/主题及跨刷新持久化 |

**此前文档称「EPUB 在无头环境渲染不出来（iframe 高度=0）」，该限制已不成立** —— 0.1.1 的显式像素 `renderTo` 修复解决了它，实测 iframe 高度 765px，EPUB 的点词、批量、翻页、进度恢复均已自动化验证。

未自动化的部分：📂 按钮走系统文件对话框（无法驱动）、真机 iPad Safari、PWA 安装、以及 DeepSeek/欧陆的**真实**响应质量（只验了链路，没验模型输出对不对）。
